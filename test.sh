...
if [ -z "$(efibootmgr | grep Boot)" ];
    then
        arch-chroot /mnt pacman --color always -Sdd grub --noconfirm
        ...
        arch-chroot /mnt pacman --color always -Sdd efibootmgr --noconfirm
...
if [ "$microcode" = "\ninitrd /amd-ucode.img" ]; then arch-chroot /mnt pacman --color always -Sdd amd-ucode --noconfirm
elif [ "$microcode" = "\ninitrd /intel-ucode.img" ]; then arch-chroot /mnt pacman --color always -Sdd intel-ucode iucode-tool --noconfirm
fi
#
...
if [ -n "$(lspci | grep -i vga | grep -i nvidia)" ]; then
    if [ -n "$(lspci | grep -i vga | grep -i nvidia | grep -E 'TU1|GA1|GV1|GP10|GM20|GM10')" ]; then
        arch-chroot /mnt pacman --color always -Sdd nvidia-dkms nvidia-utils lib32-nvidia-utils nvidia-settings opencl-nvidia lib32-opencl-nvidia opencv-cuda nvtop cuda --noconfirm
        arch-chroot /mnt sed -i 's/MODULES=()/MODULES=(nvidia nvidia_modeset nvidia_uvm nvidia_drm)/' /etc/mkinitcpio.conf
    else
        arch-chroot /mnt pacman --color always -Sdd xf86-video-nouveau --noconfirm
        arch-chroot /mnt sed -i 's/MODULES=()/MODULES=(nouveau)/' /etc/mkinitcpio.conf
    fi
elif [ -n "$(lspci | grep -i vga | grep -i 'vmware svga')" ]; then
    arch-chroot /mnt pacman --color always -Sdd virtualbox-guest-utils --noconfirm
    ...
    arch-chroot /mnt pacman --color always -Sdd xf86-video-ati xf86-video-amdgpu vulkan-radeon lib32-vulkan-radeon --noconfirm
    arch-chroot /mnt sed -i 's/MODULES=()/MODULES=(amdgpu radeon)/' /etc/mkinitcpio.conf
elif [ -n "$(lspci | grep -i vga | grep -i intel)" ]; then
    arch-chroot /mnt pacman --color always -Sdd vulkan-intel intel-media-driver libva-intel-driver --noconfirm
...
arch-chroot /mnt pacman --color always -Sdd xorg xorg-xinit xterm i3-gaps i3status perl-anyevent-i3 perl-json-xs dmenu xdm-archlinux --noconfirm
#Установка интернет программ.
echo -e "\033[36mУстановка интернет программ.\033[0m"
arch-chroot /mnt pacman --color always -Sdd firefox network-manager-applet wireless_tools telegram-desktop --noconfirm
#Установка bluetooth программ.
echo -e "\033[36mУстановка bluetooth программ.\033[0m"
arch-chroot /mnt pacman --color always -Sdd blueman bluez bluez-utils --noconfirm
#Установка нужных консольных программ.
echo -e "\033[36mУстановка нужных консольных программ.\033[0m"
arch-chroot /mnt pacman --color always -Sdd git mc htop nano dhcpcd imagemagick sysstat acpid clinfo avahi reflector go libnotify autocutsel openssh haveged dbus-broker --noconfirm
#Установка программ безопасности.
echo -e "\033[36mУстановка программ безопасности.\033[0m"
arch-chroot /mnt pacman --color always -Sdd polkit kwalletmanager kde-cli-tools xlockmore xautolock --noconfirm
...
arch-chroot /mnt pacman --color always -Sdd dmg2img gparted ark ntfs-3g dosfstools unzip smartmontools --noconfirm
#Установка файлового менеджера и дополнений.
echo -e "\033[36mУстановка файлового менеджера и дополнений.\033[0m"
arch-chroot /mnt pacman --color always -Sdd dolphin kdf filelight ifuse usbmuxd libplist libimobiledevice curlftpfs samba kimageformats ffmpegthumbnailer kdegraphics-thumbnailers qt5-imageformats kdesdk-thumbnailers ffmpegthumbs --noconfirm
#Установка программ внешнего вида.
echo -e "\033[36mУстановка программ внешнего вида.\033[0m"
arch-chroot /mnt pacman --color always -Sdd papirus-icon-theme picom redshift lxqt-panel grc flameshot notification-daemon qgnomeplatform-qt5 gnome-themes-extra archlinux-wallpaper feh conky freetype2 ttf-fantasque-sans-mono neofetch --noconfirm
#Установка программ звука.
echo -e "\033[36mУстановка программ звука.\033[0m"
arch-chroot /mnt pacman --color always -Sdd alsa-utils alsa-plugins lib32-alsa-plugins alsa-firmware alsa-card-profiles pulseaudio pulseaudio-alsa pulseaudio-bluetooth pavucontrol-qt --noconfirm
#Установка мультимедийных программ.
echo -e "\033[36mУстановка мультимедийных программ.\033[0m"
arch-chroot /mnt pacman --color always -Sdd hspell libvoikko aspell nuspell xed audacity cheese sweeper pinta vlc libreoffice-still-ru kalgebra copyq kontrast kamera kcolorchooser gwenview xreader gogglesmm meld --noconfirm
#Установка программ принтера/сканера.
echo -e "\033[36mУстановка программ принтера/сканера.\033[0m"
arch-chroot /mnt pacman --color always -Sdd sane skanlite cups cups-pdf system-config-printer --noconfirm
#Установка игр.
echo -e "\033[36mУстановка игр.\033[0m"
arch-chroot /mnt pacman --color always -Sdd steam wine winetricks wine-mono wine-gecko gamemode lib32-gamemode discord ktouch --noconfirm
#Установка свободных видео-драйверов.
echo -e "\033[36mУстановка свободных видео-драйверов.\033[0m"
arch-chroot /mnt pacman --color always -Sdd mesa lib32-mesa libva-mesa-driver mesa-vdpau --noconfirm
...
if [ -z "$(iwctl device list | awk '{print $2}' | grep wl | head -n 1)" ]; then arch-chroot /mnt ip link set "$netdev" up
        ...
        arch-chroot /mnt pacman --color always -Sdd iwd --noconfirm
