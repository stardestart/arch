...
memory { #Индикатор ОЗУ
    format = "ОЗУ: %used / %total" #Формат вывода.
    threshold_degraded = 10% #Желтый порог.
    threshold_critical = 5% #Красный порог.
    format_degraded = "ОЗУ: %used / %total" } #Формат вывода желтого/красного порога.
cpu_usage { #Использование ЦП.
    format = "↯(ЦП): %usage" } #Формат вывода.
cpu_temperature 0 { #Температура ЦП.
    format = "🌡(ЦП): %degrees°C" #Формат вывода.
    max_threshold = "70" #Красный порог.
    format_above_threshold = "🌡(ЦП): %degrees°C" #Формат вывода красного порога.
...
opacity=50
panelSize='"$(($font*4))"'
plugins=mainmenu, spacer, quicklaunch, kbindicator, volume, customcommand, customcommand2
position=Top
reserve-space=true
show-delay=0
visible-margin=true
